...
router.post("/testimonials", async (req, res) => {
  try {
    ...
    if (!name || !feedback || !email) {
      return res
        .status(400)
        .json({ error: "Name, feedback, and email are required" });
    }
    ...
    res
      .status(400)
      .json({ error: "Failed to create testimonial", details: err.message });
  }
});
...
router.delete("/testimonials/:id", async (req, res) => {
  try {
    ...
    if (!deletedTestimonial) {
      return res
        .status(404)
        .json({ error: "Testimonial not found for the provided ID" });
    }
    ...
    res
      .status(500)
      .json({ error: "Failed to delete testimonial", details: err.message });
  }
});
